...
export function QuickCreateProject() {
  ...
  useEffect(() => {
    if (!searchQuery.trim() || clienteMode !== 'search') {
      setSuggestions([]);
      return;
    }
  ...
  const handleAddServico = () =>
    setServicos(prev => [...prev, { nome: '', valor: 0 }]);
    ...
    try {
      ...
      if (clienteMode === 'search') {
        ...
        const { data: newCliente, error: clienteError } = await supabase
          .from('clientes')
          .insert([{
            nome_artistico: newClienteData.nome_artistico,
            nome_pessoal: newClienteData.nome_pessoal,
      ...
      const entregaveis = servicos.map(s => ({
        projeto_id: newProjeto.id,
        nome_servico: s.nome,
        valor: Number(s.valor),
        status_producao: 'Definicao de Escopo',
      }));
              ...
              <div style={{ display: 'flex', gap: 6, padding: 4, background: 'var(--bg-base)', borderRadius: 10 }}>
                {([
                  ...
                  <button
                    ...
                    style={{
                      flex: 1, padding: '8px 10px', borderRadius: 8, fontWeight: 700, fontSize: 12,
                      cursor: 'pointer', border: 'none', display: 'flex', alignItems: 'center',
                      justifyContent: 'center', gap: 6, transition: 'all 0.2s',
                      background: clienteMode === mode ? 'var(--bg-surface)' : 'transparent',
                      color: clienteMode === mode ? 'var(--text-primary)' : 'var(--text-muted)',
                      boxShadow: clienteMode === mode ? '0 2px 8px rgba(0,0,0,0.3)' : 'none',
                    }}
                  >
                    {icon} {label}
                  </button>
                ...
                <div ref={searchRef} style={{ position: 'relative' }}>
                  ...
                  {selectedCliente ? (
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: 8, background: 'rgba(124,58,237,0.12)', border: '1px solid rgba(124,58,237,0.4)' }}>
                      ...
                      <button type="button" onClick={() => { setSelectedCliente(null); setSearchQuery(''); }} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: 4 }}>
                        <X size={14} />
                      </button>
                    </div>
                  ) : (
                    <>
                      <div style={{ position: 'relative' }}>
                        ...
                        <input type="text" placeholder="Nome artistico, real ou telefone..." value={searchQuery} onChange={e => { setSearchQuery(e.target.value); setShowDropdown(true); }} onFocus={() => searchQuery && setShowDropdown(true)} style={{ ...inputStyle, paddingLeft: 36 }} />
                      </div>
                      {showDropdown && suggestions.length > 0 && (
                        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 10, zIndex: 100, overflow: 'hidden', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
                          {suggestions.map(c => (
                            <button key={c.id} type="button" onClick={() => { setSelectedCliente(c); setSearchQuery(''); setShowDropdown(false); }} style={{ width: '100%', padding: '10px 14px', background: 'none', border: 'none', color: 'var(--text-primary)', cursor: 'pointer', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: 2, borderBottom: '1px solid var(--border)' }} onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.04)')} onMouseLeave={e => (e.currentTarget.style.background = 'none')}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {servicos.map((servico, index) => (
                    <div key={index} style={{ display: 'flex', gap: 8, alignItems: 'center', background: 'rgba(255,255,255,0.02)', padding: '10px 12px', borderRadius: 10, border: '1px solid var(--border)' }}>
                      ...
                      <input required type="text" value={servico.nome} onChange={e => handleServicoChange(index, 'nome', e.target.value)} placeholder="Ex: Mixagem Musica X..." style={{ ...inputStyle, flex: 2, padding: '7px 10px', minWidth: 0 }} />
